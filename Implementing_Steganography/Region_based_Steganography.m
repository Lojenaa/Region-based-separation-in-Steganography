...
image_folder = 'G:\image_research\Final_001\Final'; %  Enter name of folder from which you want to upload pictures with full path
...
fileid=fopen('F:\image_back\image_processing\image\misc\New folder\message.txt','r');
data=fread(fileid);
s=char(data);
fclose(fileid);
...
file3=strcat('3vNf8','Roughregion',Nfile);
file2=strcat('3vNf8',name,'Smooth',Nfile);
file1=strcat('3vNf8',name,'Rough',Nfile);
...
folder1='F:\image_backup\output\setgoimage';
...
%Read the message from the Rough region setgoimage
for bi_1=1:1:fix(si)
...
folder1='F:\image_backup\output\org_image_s';
...
folder1='F:\image_backup\output\org_image_s1';
imwrite(sqr,fullfile(folder1,file2));

%save the orignal image
folder2='F:\image_backup\output\orignalimage';
