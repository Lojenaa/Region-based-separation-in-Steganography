...
clc;
global pp;global check_d;global check_skd2;global check_dk;global edge_find;global ede;global check_e;global check_s;global check_ek;global check_sk;global check_ekd;global check_skd;global BW2;global rk;global re;global rs;global g_string;global h_string;global g_string1;global h_string1;global changes1;global i_o;global ch1;global i_c;global i_o1;global i_c1;global im;global ch;global changes ;global dpv;global x;global y ;global r;global Rdum;global Gdum;global Bdum;global index;global index_t; global binary;global out;global m_size ;global max_in;global in;global k;%global index_t_B;global index_t_R;global index_t_G;

details=cell(fix(0),fix(0));
image_folder = 'F:\image processing\sample image\512\test\final2\final test\30 images'; %  Enter name of folder from which you want to upload pictures with full path
...
x=fix(size(im,2));
y=fix(size(im,1));
rs=0;re=0;r2=0;r1=0;rk=0;ede=0; er=0;fe=0;srk=0; ssrk=0; esrk=1;sh=1;
c=5;
in=0;
i_c=1;
i_o=1;
...
check_d=zeros(m_size,2);
check_dk=zeros(m_size,1);
edge1=cell(1,fix(0));
check_e=cell(1,fix(0));
check_es=cell(1,fix(0));
check_s=cell(1,fix(0));
check_ek=cell(1,fix(0));%filiter edge area
check_sk=cell(1,fix(0));%filiter smooth area
check_ekd=cell(1,fix(0));%used area in edge
check_skd=cell(1,fix(0));%used area in smooth


edge_find=zeros(m_size,m_size);
mi_a=zeros(1,fix(0));
ma_a=zeros(1,fix(0));
max_in=zeros(1,fix(0));

sm_edge=zeros(1,fix(0));
edge1_s=cell(1,fix(0));
n=1;
...
for Ya=1:1:fix(y/m_size)
    for Xa=1:1:fix(x/m_size)
        ...
        wi=temp4_bn((Xa*m_size)-(m_size-1),Xa*m_size,(Ya*m_size)-(m_size-1),Ya*m_size);    
    end
    
end

%Store the edge Block index 
smk=1;
for sYa=1:1:fix(y/m_size)
    for sXa=1:1:fix(x/m_size)
         if size(check_ek,2)>smk
          sm=check_ek{1,smk};
        ...
        if size(check_es,2)>sh
           ...
           if(smo(2)==0&&smo(1)==0)
             ...
             eyl=sYa*m_size-(m_size-1);
              ...
              ssrk=ssrk+1;
                check_sk{1,ssrk}=[ num2str(sXa) ',' num2str(sYa)];
...
fileid=fopen('f:\image processing\image\misc\New folder\message.txt','r');
data=fread(fileid);
s=char(data);
fclose(fileid);

name=num2str(dpv);
file=strcat('3Nf8',name,Nfile);
file3=strcat('3vNf8','edgeArea',Nfile);
file2=strcat('3vNf8',name,'smooth',Nfile);
file1=strcat('3vNf8',name,'edge',Nfile);
kk1=[file,num2str(dpv)];
disp('................dpv.................');
disp(kk1);
disp('................dpv.................');
...
xo=1;% insert the message into edge area
for bi=1:1:cek
  ...
  sqr=insertShape(sqr,'Rectangle',[ xl  yl  m_size m_size],'Color',{'green'},'LineWidth',1);
...
% save the edge area setgoimage
folder1='f:\image processing\image\output\setgoimage';
...
%Read the message from the edge area setgoimage
for bi_1=1:1:fix(si)
...
% insert the message into smooth area
for bs=1:1:csk
  ...
  sqr=insertShape(sqr,'Rectangle',[ sx  sy  m_size m_size],'Color',{'red'},'LineWidth',1);
...
% save the smooth area setgoimage
folder1='f:\image processing\image\output\org_image_s';
...
sni=1;
%Read the message from the smooth area setgoimage
          ...
          for sj1=1:1:m_size
             sin_val=line_inS(si1,sj1);
                   ...
                   if sni<=size(changes1,2);
                    ...
                    sni=sni+1;
                    sin_to= expant_1_s(sin_val,sc_bit);
...
folder='f:\image processing\image\output\selectblock';
imwrite(sqr,fullfile(folder,file));

folder2='f:\image processing\image\output\orignalimage';
